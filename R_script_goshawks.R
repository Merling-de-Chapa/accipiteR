...
test_react_int <- fitme(reaction ~ Habitat*Age + No_nestlings + Laying_begin_day + Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
...
# 2. Error structure
testSpatialAutocorrelation(sim_react, y = jitter(goshawk_nest$Latitude, factor = 0.001), x = jitter(goshawk_nest$Longitude, factor = 0.001))


test_react_0 <- fitme(reaction~1+(1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_react_0)

## effects
test_no_Hab_age<- fitme(reaction ~ No_nestlings +  Laying_begin_day + Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_no_Hab_age)

test_noHabitat <- fitme(reaction ~ Age + No_nestlings +  Laying_begin_day + Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_noHabitat)

test_nonest <- fitme(reaction ~ Habitat*Age + Laying_begin_day + Year +  Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_nonest)

test_noYear <- fitme(reaction ~ Habitat*Age + No_nestlings + Laying_begin_day + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_noYear)

test_noDay <- fitme(reaction ~ Habitat*Age + No_nestlings +  Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_noDay)

test_noRain <- fitme(reaction ~ Habitat*Age + No_nestlings + Laying_begin_day + Year + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
anova(test_react_int, test_noRain)

test_noYoung <- fitme(reaction ~ Habitat + Rainfall + No_nestlings + Laying_begin_day + Year + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
...
## ODDS 
rural_young <- data.frame(Habitat = "rural", Age = "young", No_nestlings = 0, Laying_begin_day = 0, Year = "2015", Rainfall = 0, Location = "new", Territory = "new") 
urban_young <- data.frame(Habitat = "urban", Age = "young", No_nestlings = 0, Laying_begin_day = 0, Year = "2015", Rainfall = 0, Location = "new", Territory = "new") 
rural_old   <- data.frame(Habitat = "rural", Age = "old",   No_nestlings = 0, Laying_begin_day = 0, Year = "2015", Rainfall = 0, Location = "new", Territory = "new") 
urban_old   <- data.frame(Habitat = "urban", Age = "old",   No_nestlings = 0, Laying_begin_day = 0, Year = "2015", Rainfall = 0, Location = "new", Territory = "new") 
...
round(odd_ratio_urban_young_vs_rural_young <- OR(prob_urban_young, prob_rural_young), 2)
round(odd_ratio_urban_old_vs_rural_old <- OR(prob_urban_old, prob_rural_old), 2)
round(odd_ratio_urban_young_vs_urban_old <- OR(prob_urban_young, prob_urban_old), 2)
round(odd_ratio_rural_young_vs_rural_old <- OR(prob_rural_young, prob_rural_old), 2)
...
test_no_int <- fitme(reaction ~ Habitat+Age + No_nestlings + Laying_begin_day + Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
round(exp(fixef(test_no_int)["Habitaturban"]), 2)

test_no_Habitat <- fitme(reaction ~ Age + No_nestlings + Laying_begin_day + Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")
test_no_Age <- fitme(reaction ~ Habitat + No_nestlings + Laying_begin_day + Year + Rainfall + (1|Location/Territory), family=binomial(link=logit), data= goshawk_nest, method = "PQL/L")


spaMM.options(nb_cores = 3, separation_max = 1) 
library(doSNOW)
anova(test_int, test_no_int, boot.repl = 1000)
...
pred_urban_young <- predict(test_no_int, newdata = expand.grid(Laying_begin_day=median(goshawk_nest$Laying_begin_day, na.rm = TRUE), Age="young", Rainfall = median(goshawk_nest$Rainfall, na.rm = TRUE), No_nestlings = 3, Year = "2015", Habitat = "urban"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_1 <- data.frame(pred = pred_urban_young, attr(pred_urban_young, "intervals"), Habitat = "urban", Age = "young")
pred_1

pred_urban_old <- predict(test_no_int, newdata = expand.grid(Laying_begin_day=median(goshawk_nest$Laying_begin_day, na.rm = TRUE), Age="old", Rainfall = median(goshawk_nest$Rainfall, na.rm = TRUE), No_nestlings = 3, Year = "2015", Habitat = "urban"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_2 <- data.frame(pred = pred_urban_old, attr(pred_urban_old, "intervals"), Habitat = "urban", Age = "old")

pred_rural_young <- predict(test_no_int, newdata = expand.grid(Laying_begin_day=median(goshawk_nest$Laying_begin_day, na.rm = TRUE), Age="young", Rainfall = median(goshawk_nest$Rainfall, na.rm = TRUE), No_nestlings = 3, Year = "2015", Habitat = "rural"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_3 <- data.frame(pred = pred_rural_young, attr(pred_rural_young, "intervals"), Habitat = "rural", Age = "young")

pred_rural_old <- predict(test_no_int, newdata = expand.grid(Laying_begin_day=median(goshawk_nest$Laying_begin_day, na.rm = TRUE), Age="old", Rainfall = median(goshawk_nest$Rainfall, na.rm = TRUE), No_nestlings = 3, Year = "2015", Habitat = "rural"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
...
ggsave(filename = "reaction_Habitat.pdf", width = 8, height =  7)
...
pred_lay_obj <- predict(test_no_int, newdata = expand.grid(Laying_begin_day=predictor_lay, Age = "young", Rainfall = median(goshawk_nest$Rainfall, na.rm = TRUE), No_nestlings = 3, Year = "2015", Habitat = "urban"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_lay <- data.frame(pred = pred_lay_obj, attr(pred_lay_obj, "intervals"), lay = predictor_lay)

predictor_lay2 <- seq(min(goshawk_nest$Laying_begin_day, na.rm = TRUE), max(goshawk_nest$Laying_begin_day, na.rm = TRUE), length = 30)
pred_lay_obj2 <- predict(test_no_int, newdata = expand.grid(Laying_begin_day=predictor_lay2, Age = "young", Rainfall = median(goshawk_nest$Rainfall, na.rm = TRUE), No_nestlings = 3, Year = "2015", Habitat = "rural"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_lay2 <- data.frame(pred = pred_lay_obj2, attr(pred_lay_obj2, "intervals"), lay2 = predictor_lay2)


pdf("plot_prediction1.pdf", width = 10, height = 7)

plot(pred_lay$pred ~ pred_lay$lay, type= "l", lwd = 2, ylab = "Proportion of females reacting", las = 1, ylim = c(0, 1), xlab = "Laying date", axes = FALSE)
...
lines(pred_lay2$pred ~ pred_lay2$lay2, type= "l", lwd = 2, ylab = "Proportion of females reacting", las = 1, ylim = c(0, 1), xlab = "Laying date",  col="grey")
...
cut_off2$species  <- factor(cut_off2$species, levels = levels(cut_off2$species)[c(1,22, 12, 20, 9, 3, 10, 14, 6, 4, 8, 5, 18, 15, 13, 21, 7, 16, 19, 17, 2, 11)])
...
#### location######
GLMM_pigeon_SpaMM_loc <- fitme(cbind(Pigeon, Rest)~Habitat + (1|Location/nest_side),family=binomial(link="logit"), data=pigeon, method="PQL/L")
...
GLMM_0_loc <- fitme(cbind(Pigeon, Rest)~1 + (1|Location/nest_side),family=binomial(link="logit"), data=pigeon, method = "PQL/L")
...
testSpatialAutocorrelation(sim_diversity, y = jitter(species$Latitude, factor = 0.001), x = jitter(species$Longitude, factor = 0.001))
...
glm_species_spaMM <- fitme(Species_richness_nest ~ Habitat+(1|Location), family = Tnegbin(), data = species)
...
# 2. Error structure
# spatial autocorrelation 
testSpatialAutocorrelation(sim_species, y = jitter(species$Latitude, factor = 0.001), x = jitter(species$Longitude, factor = 0.001))
...
#### location#####
lmm_laying2 <- fitme(Laying_begin_day ~ Habitat + Temp_breeding_begin  + (1|Location/Territory), family = gaussian, data = goshawk_nest2)
...
# 3. Error structure
testSpatialAutocorrelation(sim_laying, x=jitter(goshawk_nest2$Longitude), y=jitter(goshawk_nest2$Latitude))
...
lmm_0 <- fitme(Laying_begin_day ~ 1+(1|Location/Territory), family = gaussian, data = goshawk_nest2)
...
lmm_laying_no_Habitat <- fitme(Laying_begin_day ~ Temp_breeding_begin + (1|Location/Territory), family = gaussian, data = goshawk_nest2)
anova(lmm_laying2, lmm_laying_no_Habitat)

lmm_laying_no_Temp <- fitme(Laying_begin_day ~ Habitat + (1|Location/Territory), family = gaussian, data = goshawk_nest2)
...
glmm_nestlings<-fitme(No_nestlings_binary ~ Habitat + Laying_begin_day + Temp_breeding_begin + (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
...
testSpatialAutocorrelation(sim_nestling, y = jitter(goshawk_nest3$Latitude, factor = 0.001), x = jitter(goshawk_nes3t$Longitude, factor = 0.001))

# 3. Overdispersion
# not necessary binary

glmm_0_spaMM<-fitme(No_nestlings_binary ~ 1 +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
anova(glmm_nestlings, glmm_0_spaMM)

glmm_nestlings_no_Habitat<-fitme(No_nestlings_binary ~ Laying_begin_day +Temp_breeding_begin +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
anova(glmm_nestlings, glmm_nestlings_no_Habitat)

glmm_nestlings_no_day<-fitme(No_nestlings_binary ~ Habitat + Temp_breeding_begin +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
anova(glmm_nestlings, glmm_nestlings_no_day)

glmm_nestlings_no_Temp<-fitme(No_nestlings_binary ~ Habitat + Laying_begin_day +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
...
predictor_day <- seq(min(goshawk_nest3$Laying_begin_day, na.rm = TRUE), max(goshawk_nest3$Laying_begin_day, na.rm = TRUE), length = 30)
pred_day_obj <- predict(glmm_nestlings, newdata = expand.grid(Laying_begin_day=predictor_day,Habitat = "urban", Temp_breeding_begin = median(goshawk_nest3$Temp_breeding_begin, na.rm = TRUE)), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
...
pred_day_obj2 <- predict(glmm_nestlings, newdata = expand.grid(Laying_begin_day=predictor_day2,Habitat = "rural", Temp_breeding_begin = median(goshawk_nest3$Temp_breeding_begin, na.rm = TRUE)), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_day2 <- data.frame(pred2 = pred_day_obj2, attr(pred_day_obj2, "intervals"), day2 = predictor_day2)

pdf("prediction_plot2.pdf", width = 10, height = 7)

plot(pred_day$pred ~ pred_day$day, type= "l", lwd = 2, ylab = "Proportion of large clutches", las = 1, ylim = c(0, 1), xlab = "Laying date", axes = FALSE)
...
lines(pred_day2$pred2 ~ pred_day2$day2, type= "l", lwd = 2, ylab = "Proportion of large clutches", las = 1, ylim = c(0, 1), xlab = "Laying date", col = "grey")
...
glmm_nestlings2<-fitme(No_nestlings_binary ~ Habitat + Temp_breeding_begin + (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
...
# 2. Error structure
#testTemporalAutocorrelation(sim_nestling, time = goshawk_nest$Laying_begin_day + ifelse(goshawk_nest$Year == "2016", 365, 0))
testSpatialAutocorrelation(sim_glm2, y = jitter(goshawk_nest3$Latitude, factor = 0.001), x = jitter(goshawk_nest3$Longitude, factor = 0.001))

glmm_0_spaMM2<-fitme(No_nestlings_binary ~ 1 +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
...
glmm_nestlings_no_Habitat2<-fitme(No_nestlings_binary ~ Temp_breeding_begin +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
anova(glmm_nestlings2, glmm_nestlings_no_Habitat2)

glmm_nestlings_no_Temp2<-fitme(No_nestlings_binary ~ Habitat +  (1|Location/Territory), family = binomial(), data= goshawk_nest3, method = "PQL/L")
...
##
glmm_tricho_logit <- fitme(Prevalence ~ Habitat + Age + No_nestlings +  Sex + Temp_age + Year + laying_day +(1|Location/Territory), family=binomial(link="logit"), data=goshawk_urban, method="PQL/L")
...
testSpatialAutocorrelation(sim_tricho, y = jitter(goshawk_urban$Latitude, factor = 0.001), x = jitter(goshawk_urban$Longitude, factor = 0.001))

# test for overdispersion and zero inflation
# not necessary for binary data!!

glmm_tricho_logit_0<-fitme(Prevalence~1+(1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L") 
...
#test effects
glmm_tricho_logit_noHabitat <- fitme(Prevalence ~  Age + No_nestlings + Sex + Temp_age + Year + laying_day + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_tricho_logit, glmm_tricho_logit_noHabitat)

glmm_tricho_logit_nonest <- fitme(Prevalence ~ Habitat + Age + Sex + Temp_age + Year + laying_day + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_tricho_logit, glmm_tricho_logit_nonest)

glmm_tricho_logit_noage <- fitme(Prevalence ~ Habitat + No_nestlings + Sex + Temp_age + Year + laying_day + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_tricho_logit, glmm_tricho_logit_noage)

glmm_tricho_logit_noTemp <- fitme(Prevalence ~ Habitat + Age + No_nestlings + Sex + Year + laying_day + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_tricho_logit, glmm_tricho_logit_noTemp)

glmm_tricho_logit_noSex <- fitme(Prevalence ~ Habitat + Age + No_nestlings + Year + laying_day + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_tricho_logit, glmm_tricho_logit_noSex)

glmm_tricho_logit_noYear <- fitme(Prevalence ~ Habitat + Age + No_nestlings + Sex + Temp_age + laying_day + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_tricho_logit, glmm_tricho_logit_noYear)

glmm_tricho_logit_noDay <- fitme(Prevalence ~ Habitat + Age + No_nestlings + Sex + Temp_age + Year + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
...
glmm_clinical <- fitme(Clinical_binary ~ Habitat+Age+Sex+Year+laying_day+No_nestlings+Temp_age + (1|Location/Territory), family=binomial(link="logit"), data=goshawk_urban, method="PQL/L")
glmm_0 <- fitme(Clinical_binary ~ 1+(1|Location/Territory), family=binomial(link="logit"), data=goshawk_urban, method="PQL/L")
...
# 2. Error structure
testTemporalAutocorrelation(sim_tricho, time = goshawk_urban$laying_day + ifelse(goshawk_urban$Year == "2015", 365, 0) + ifelse(goshawk_urban$Year == "2016", 730, 0))
testSpatialAutocorrelation(sim_clinical, y = jitter(goshawk_urban$Latitude, factor = 0.001), x = jitter(goshawk_urban$Longitude, factor = 0.001))
...
#test effects
glmm_clinical_noHabitat <- fitme(Clinical_binary ~  Age + No_nestlings + Sex + Year + laying_day + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_clinical, glmm_clinical_noHabitat)

glmm_clinical_nonest <- fitme(Clinical_binary ~ Habitat + Age + Sex + Year + laying_day + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_clinical, glmm_clinical_nonest)

glmm_clinical_noage <- fitme(Clinical_binary ~ Habitat + No_nestlings + Sex + Year + laying_day + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_clinical, glmm_clinical_noage)

glmm_clinical_noSex <- fitme(Clinical_binary ~ Habitat + Age + No_nestlings + Year + laying_day + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_clinical, glmm_clinical_noSex)

glmm_cinical_noTemp <- fitme(Clinical_binary ~ Habitat + Age + No_nestlings + Sex + Year + laying_day +  (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_clinical, glmm_clinical_noTemp)

glmm_clinical_noYear <- fitme(Clinical_binary ~ Habitat + Age + No_nestlings + Sex + laying_day + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
anova(glmm_clinical, glmm_clinical_noYear)

glmm_clinical_noDay <- fitme(Clinical_binary ~ Habitat + Age + No_nestlings + Sex + Year + Temp_age + (1|Location/Territory), family=binomial(link=logit), data= goshawk_urban, method = "PQL/L")
...
# Laying day
predictor_lay3 <- seq(min(goshawk_urban$laying_day, na.rm = TRUE), max(goshawk_urban$laying_day, na.rm = TRUE), length = 30)
pred_lay_obj3 <- predict(glmm_clinical, newdata = expand.grid(laying_day=predictor_lay3, No_nestlings = 3, Year = "2015", Sex="female",Age = median(goshawk_urban$Age, na.rm = TRUE), Habitat = "urban", Temp_age = median(goshawk_urban$Temp_age, na.rm = TRUE)), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_lay3 <- data.frame(pred = pred_lay_obj3, attr(pred_lay_obj3, "intervals"), lay = predictor_lay3)

predictor_lay4 <- seq(min(goshawk_urban$laying_day, na.rm = TRUE), max(goshawk_urban$laying_day, na.rm = TRUE), length = 30)
pred_lay_obj4 <- predict(glmm_clinical, newdata = expand.grid(laying_day=predictor_lay4, No_nestlings = 3, Year = "2015", Sex="female",Age = median(goshawk_urban$Age, na.rm = TRUE), Habitat = "rural", Temp_age = median(goshawk_urban$Temp_age, na.rm = TRUE)), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_lay4 <- data.frame(pred4 = pred_lay_obj4, attr(pred_lay_obj4, "intervals"), lay4 = predictor_lay4)


pdf("prediction_plot3.pdf", width = 10, height = 7) 

plot(pred_lay3$pred ~ pred_lay3$lay, type= "l", lwd = 2, ylab = "Proportion of nestlings presenting clinical signs", las = 1, ylim = c(0, 1), xlab = "Laying date", axes = FALSE)
...
lines(pred_lay4$pred4 ~ pred_lay4$lay4, type= "l", lwd = 2, ylab = "Proportion of nestlings presenting clinical signs", las = 1, ylim = c(0, 1), xlab = "Laying date", col = "grey")
...
#### Age
predictor_age <- seq(min(goshawk_urban$Age, na.rm = TRUE), max(goshawk_urban$Age, na.rm = TRUE), length = 30)
pred_age_obj <- predict(glmm_clinical, newdata = expand.grid(Age=predictor_age, No_nestlings = 3, Year = "2015", Sex="female",laying_day = median(goshawk_urban$laying_day, na.rm = TRUE), Temp_age = median(goshawk_urban$Temp_age, na.rm = TRUE), Habitat = "urban"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_age <- data.frame(pred = pred_age_obj, attr(pred_age_obj, "intervals"), age = predictor_age)

predictor_age2 <- seq(min(goshawk_urban$Age, na.rm = TRUE), max(goshawk_urban$Age, na.rm = TRUE), length = 30)
pred_age_obj2 <- predict(glmm_clinical, newdata = expand.grid(Age=predictor_age2, No_nestlings = 3, Year = "2015", Sex="female",laying_day = median(goshawk_urban$laying_day, na.rm = TRUE), Temp_age = median(goshawk_urban$Temp_age, na.rm = TRUE), Habitat = "rural"), re.form = NA, intervals = "predVar") # predVar= SD of Prediction^2
pred_age2 <- data.frame(pred2 = pred_age_obj2, attr(pred_age_obj2, "intervals"), age2 = predictor_age2)


pdf("preidction_plot4.pdf", width = 10, height = 7)

plot(pred_age$pred ~ pred_age$age, type= "l", lwd = 2, ylab = "Proportion of nestlings presenting clinical signs", las = 1, ylim = c(0, 1), xlab = "Age of nestlings")
...
lines(pred_age2$pred2 ~ pred_age2$age2, type= "l", lwd = 2, ylab = "Proportion of nestlings presenting clinical signs", las = 1, ylim = c(0, 1), xlab = "Age of nestlings", col = "grey")
...
glm_death <- fitme(Trichomonosiasis ~ location + age + sex, family=binomial(link="logit"), data=death, method= "PQL/L")
...
GLM_0<-fitme(Trichomonosiasis~1, family=binomial(link=logit), data=death, method="PQL/L")
...
glm_death_no_loc <- fitme(Trichomonosiasis ~ age + sex, family=binomial(link="logit"), data=death, method="PQL/L")
anova(glm_death_no_loc, glm_death)

glm_death_no_age <- fitme(Trichomonosiasis ~ location + sex, family=binomial(link="logit"), data=death, method="PQL/L")
anova(glm_death_no_age, glm_death)

glm_death_no_sex <- fitme(Trichomonosiasis ~ location + age, family=binomial(link="logit"), data=death, method="PQL/L")
...
glm_death2 <- fitme(Trauma_window ~ location + sex, family=binomial(link="logit"), data=death, method="PQL/L")
...
# test againt zero model
GLM_0<-fitme(Trauma_window~1, family=binomial(link=logit), data=death, method="PQL/L")
...
glm_death_no_loc2 <- fitme(Trauma_window ~  sex, family=binomial(link="logit"), data=death, method="PQL/L")
anova(glm_death_no_loc2, glm_death2)

glm_death_no_sex2 <- fitme(Trauma_window ~ location, family=binomial(link="logit"), data=death, method="PQL/L")
  ...
  mutate(Cause_of_death = ifelse(Cause_of_death == "Unknwon", "Unknown", 
                                 ifelse(Cause_of_death == "Trichmononosis", "Trichomonosis", 
                                        ifelse(Cause_of_death == "Trauma (unknwon)" | 
                                                 Cause_of_death == "Trauma (unkown)", "Trauma (unknown)", as.character(Cause_of_death)))))  %>%
...
ggplot(data_plot2, aes(y = prop, x = Cause_of_death, fill = location, ymin = CI_lwr, ymax = CI_upr))+ 
  ...
  theme(panel.grid.minor = element_blank(), 
        ...
        axis.text.x = element_text(angle = 90, hjust =  1), 
        panel.grid.minor.x = element_blank()) 
ggsave(filename = "cause_of_death_plot.pdf", width = 10, height = 8)
...
ggplot(goshawk_imper, aes(x = Habitat, y = imperviousness2500)) +
  ...
  theme_bw()+
  ylab("\n Imperviousness (%) \n")
ggsave(filename = "box_plot1.pdf", width = 8, height = 8)
